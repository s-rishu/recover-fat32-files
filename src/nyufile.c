...
void printFSInfo(char *disk){
    ...
    if(fd == -1){
        printf("Error while reading disk file.");
        exit(0);
    }
    struct BootEntry *boot;
    boot = (struct BootEntry *)mmap(0, sizeof(struct BootEntry), PROT_READ, MAP_PRIVATE, fd, 0);
    printf("Number of FATs = %d\nNumber of bytes per sector = %d\nNumber of sectors per cluster = %d\nNumber of reserved sectors = %d", boot->BPB_NumFATs, boot->BPB_BytsPerSec, boot->BPB_SecPerClus, boot->BPB_RsvdSecCnt);
}
